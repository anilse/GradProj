...
#TRAIN_CSV = "hasyv2/verification-task/train.csv"
#TEST_CSV = "hasyv2/verification-task/test-v3.csv"
DATA_FOLDER = "hasyv2"
TEST_FOLDER = "hasyv2_test"
...
def loadTestData():
    ...
    ### corresponding integer value eg. (0, 1, 2...)
    for i, CLASS in enumerate(CLASSES):
        images = load_images_from_folder(TEST_FOLDER + '/' + CLASS)         # Load images from folder
        print(CLASS +" loaded for validation")
        data = np.concatenate((data,images), axis=0)    # Add features (images) to data variable
        # print("concatenated")
        label = len(images) * [i]                       # Create a list of the feature labels the length of the number of images
        labels = np.concatenate((labels, label), axis=0)# Append the list of labels to the labels variable
    ...
    # Split data into test and train sets
    cutoff = int(len(data))          # Determine the index at which to split the dataset into test and train
    x_test = data[0:cutoff]                            # Training features
    y_test = labels[0:cutoff]                          # Training labels
    np.save('test',x_test)
...
x_test = x_test.reshape(x_test.shape[0],IMG_ROWS, IMG_COLS,1)  # Reshape x_test
...
x_test = x_test.astype('float32')     # Convert x_train to float32
